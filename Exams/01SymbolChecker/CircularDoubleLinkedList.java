...
public class CircularDoubleLinkedList<T> implements Iterable<T> {
	...
	void goToPrev() { if (current != null) current = current.prevLink; }
	...
	final void insertAfterCurrent(T ... data) {
		...
		if (head == null) {
			current = head = new ListNode();
			head.prevLink = head; // head is undefined when construted :/
		}
	...
	public Iterator<T> iterator() {
		return new Iterator<T>() {
			...
			public boolean hasNext() {
				return (firstRun || temp != head) && temp != null && temp.data != null;
			}
